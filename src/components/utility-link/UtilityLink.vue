<!-- 親コンポーネントの例
  ...
  --通常版
  <UtilityLink href="https://example.com" target="_blank">
    外部サイトを見る
  </UtilityLink>
...
-->
<template>
  <component :is="asChild ? 'span' : 'a'" v-bind="$attrs" :class="[utilityLinkStyle, $attrs.class]">
    <slot />
    <template v-if="showExternalIcon">
      <svg
        :aria-label="iconAriaLabel"
        role="img"
        class="mb-1 ml-1 inline"
        fill="none"
        height="13"
        viewBox="0 0 12 13"
        width="14"
...
const showExternalIcon = computed(() => {
  return !props.asChild && attrs.target === '_blank'
})

const iconAriaLabel = computed(() => props.iconAriaLabel ?? '新規タブで開きます')

const utilityLinkStyle = `!text-solid-gray-800 text-dns-16N-130 underline underline-offset-[calc(3/16*1rem)]
  hover:decoration-[calc(3/16*1rem)]
  focus-visible:rounded-4 focus-visible:outline focus-visible:outline-4 focus-visible:outline-black
